...
outSet = Set.foldl (\chans state -> Set.union chans (out state)) Set.empty

inSet :: Ord b => Set (State a b) -> Set b
inSet = Set.foldl (\chans state -> Set.union chans (inp state)) Set.empty
         ...
         in case Map.lookup initial' fullStateMap of
                ...
                Just (ini,outi,tmapi) ->
                    let statesandmap = getStatesAndMap fullStateMap
                     in uncurry (Aut (State initial' ini outi tmapi)) statesandmap inps' outs'
  where
    ...
    getStatesAndMap =
        Map.foldlWithKey (\setandmap key val -> case val of
            (ins,outs,tmaps) ->
                ...
                in bimap (Set.insert state) (Map.insert key state) setandmap) (Set.empty,Map.empty)
    ...
    mergeMaps f (ni,no,nm) (oi,oo,om) =
        case Map.toList nm of
          [] -> error "stautdef empty Map: this error was written in a refactor, this function used to crash on an empty Map but I'm not sure why"
          -- [] -> (Set.union ni oi, Set.union no oo, Map.insert c s om)
          (c,s):_ -> case Map.lookup c om of
...
printCompRel compRel = List.intercalate "\n" (Set.toList $ Set.map (\t -> "(s" ++ show (sid $ fst t) ++ ", s" ++ show (sid $ snd t) ++ ")") (Set.filter (uncurry (/=)) compRel))
...
adgAutFromAutomaton aut delta = let
    ...
    in case Automaton.stateConf aut of
        ...
        ForbiddenDet -> error "Forbidden"
        UnderspecDet -> error "Underspecified"
    where
