...
-}
makeTester :: (StepSemantics m loc q t tdest act, TestChoice i act, BoundedConfiguration m, Ord q, Ord (m q), Ord act) =>
...
--                Left (i, state') -> Left (i, (spec, testController { testControllerState = state' }))
        makeUpdate :: (StepSemantics m loc q t tdest act, BoundedConfiguration m, Refusable act, Ord q, Ord (m q), Ord act) =>
...
-}
runTester :: (StepSemantics m loc q t tdest act, TestChoice i act, BoundedConfiguration m, Ord q, Ord (m q), Ord act) =>
