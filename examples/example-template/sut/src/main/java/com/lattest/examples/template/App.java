...
public class App {
	...
	static class Implementation {
		...
		private Integer generateOutput() {
			switch (state) {
			case PickEither:
			...
			while (true) {
				...
				try (JsonParser parser = jf.createParser((InputStream) in)) {
					...
					do {
						...
						for (Integer output = impl.generateOutput(); output != null; output = impl.generateOutput()) {
						...
						if (jsonInput != null) {
							try {
								input = jsonInput.intValue();
								System.out.println("parsed input as int: " + jsonInput);
